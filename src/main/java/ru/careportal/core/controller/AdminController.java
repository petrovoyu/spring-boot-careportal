...
public class AdminController {
    ...
    public String showUser(Model model, @PathVariable("id") Long id,
        ...
        switch (role) {
            case ROLE_PATIENT:
                showPatient(model, (Patient) user);
                break;
            case ROLE_DOCTOR:
                showDoctor(model, (Doctor) user, id, userChangesDto);
                break;
        }
        return "baseTemplate";
    }
    ...
    private void showDoctor(Model model, Doctor doctor, Long id, UserChangesDto userChangesDto) {
        ...
        List<UserDto> usersDto = usersWithoutDoc.stream()
                .map(userWithoutDoc -> new UserDto(userWithoutDoc))
                .collect(Collectors.toList());
        userChangesDto.setUsers(usersDto);
        model.addAttribute("id", id);
        model.addAttribute("userChangesDto", userChangesDto);
        List<UserDto> patients = doctor.getPatients().stream()
                .map(patient -> new UserDto(patient))
                .collect(Collectors.toList());
    ...
    private void showPatient(Model model, Patient user) {
